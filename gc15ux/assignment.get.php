	...
	*/
	...
	if ((!empty($_POST)) && (isset($_POST['task'])))
	{
		...
		if ((isset($_POST['confirm'])) && (strtoupper($_POST['confirm']) == "YES")) {
			if (!userHasGivenGCConsent($loggedInUser)) { header("Location: consent.php?assignTask=".$_POST['task']); die(); }
			$assignments = userGetGCAssignments($loggedInUser, $tid);
			if (count($assignments) == 0) {
				$assignments = userAssignGCQueries($loggedInUser, $tid);
				$assignment_results = userInitGCResults($loggedInUser, $tid);
			}
